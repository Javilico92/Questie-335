...
function _Qframe:OnClick(button)
    ...
    if uiMapId and IsControlKeyDown() and TomTom and TomTom.AddWaypoint then
        ...
        if Questie.db.char._tom_waypoint and TomTom.RemoveWaypoint then
            ...
            add = QuestieCompat.Is335 or (waypoint[1] ~= uiMapId or waypoint[2] ~= x or waypoint[3] ~= y or waypoint.title ~= title or waypoint.from ~= "Questie")
        end

        -- Add waypoint
        if QuestieCompat.Is335 then
            Questie.db.char._tom_waypoint = QuestieCompat.TomTom_AddWaypoint(title, uiMapId, self.x, self.y)
        else
